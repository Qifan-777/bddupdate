...
class ProbabilityAnalysis : public Analysis {
 public:
  ...
  /// @returns BDD node probabilities if BDD is used for probability analysis.
  ///          Each pair contains the event name and its node probability.
  virtual const std::vector<std::pair<std::string, double>>* bdd_node_probabilities()
      const {
    return nullptr;
  }
...
class ProbabilityAnalyzer<Bdd> : public ProbabilityAnalyzerBase {
 public:
  ...
  const std::vector<std::pair<std::string, double>>* bdd_node_probabilities()
      const override {
 ...
 private:
  ...
  /// Collects BDD node probabilities after calculation.
  ///
  /// @param[in] vertex  The root vertex of a function graph.
  /// @param[in] mark  A flag to mark traversed vertices.
  void CollectNodeProbabilities(const Bdd::VertexPtr& vertex,
                                bool mark) noexcept;
