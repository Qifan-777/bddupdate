...
void ProbabilityAnalyzer<Bdd>::CollectNodeProbabilities(
    const Bdd::VertexPtr& vertex, bool mark) noexcept {
  if (vertex->terminal())
    return;
  Ite& ite = Ite::Ref(vertex);
  if (ite.mark() == mark)
  ...
  if (ite.module()) {
    const Bdd::Function& res = bdd_graph_->modules().find(ite.index())->second;
    CollectNodeProbabilities(res.vertex, mark);
  }
  CollectNodeProbabilities(ite.high(), mark);
  CollectNodeProbabilities(ite.low(), mark);
}
